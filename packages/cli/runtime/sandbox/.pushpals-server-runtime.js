...
class CompletionQueue {
  ...
  reconcileRecoverableTrustedValidationFailures() {
    const requiredTables = ["jobs", "job_validation_runs", "job_terminal_diagnostics"];
    const present = this.db.prepare(`SELECT name FROM sqlite_master
         WHERE type = 'table' AND name IN ('jobs', 'job_validation_runs', 'job_terminal_diagnostics')`).all();
    ...
    for (const row of rows) {
      if (!trustedValidationCommandKey(row.failedCommand) || trustedValidationCommandKey(row.failedCommand) !== trustedValidationCommandKey(row.passedCommand)) {
        continue;
      }
    ...
    if (eligible.size === 0)
      return;
    const now = new Date().toISOString();
    const tx = this.db.transaction(() => this.restoreTrustedValidationBlockedCompletions([...eligible.values()], now));
    const recovered = tx();
    if (recovered.jobIds.length > 0) {
      console.log(`[Server] Startup recovery requeued ${recovered.jobIds.length} retained trusted-validation candidate(s): ${recovered.jobIds.join(", ")}`);
    }
  }
  ...
  markProcessedAndFinalizeJob(completionId, prUrl, trustedTiming, trustedReportInput, pusherId) {
    ...
    const tx = this.db.transaction(() => {
      ...
      const recovered = this.requeueTrustedValidationBlockedCompletionsAfterPass(completion, trustedReport, now);
  ...
  requeueTrustedValidationBlockedCompletionsAfterPass(passingCompletion, report, now) {
    if (!passingCompletion.trustedValidationCommandsJson || !report) {
      return { completionIds: [], jobIds: [] };
    }
    const requested = normalizeTrustedValidationCommands(passingCompletion.trustedValidationCommandsJson);
    if (!requested.ok)
      return { completionIds: [], jobIds: [] };
  ...
  restoreTrustedValidationBlockedCompletions(candidates, now) {
    const completionIds = [];
    const jobIds = [];
    for (const row of candidates) {
      const completionUpdate = this.db.prepare(`UPDATE completions
           ...
           WHERE id = ?
             AND status = 'failed'
             AND trustedValidationRecoveryAttempts < ?`).run(now, row.completionId, TRUSTED_VALIDATION_RECOVERY_MAX_ATTEMPTS);
      if (completionUpdate.changes === 0)
        continue;
      const jobUpdate = this.db.prepare(`UPDATE jobs
           SET status = 'finalizing',
               error = NULL,
               completedAt = NULL,
               failedAt = NULL,
               ...
               durationMs = NULL,
               updatedAt = ?
           WHERE id = ? AND status = 'publish_blocked'`).run(now, row.jobId);
      if (jobUpdate.changes === 0) {
        throw new Error(`Failed to restore parent job ${row.jobId} while requeueing completion ${row.completionId}`);
      }
...
${selected.failureFingerprint}`).slice(0, 16) : validationIncidentDigest(selected.command, failureClass, sample);
  ...
  evaluateEligibility(body) {
    ...
    const results = candidates.map((raw) => {
      ...
      if (!requiredValidationRepair && targetPaths.some((candidateTarget) => activeTargetPaths.some((activeTarget) => objectiveTargetPathsOverlap(candidateTarget, activeTarget)))) {
        return {
          candidate_id: candidateId,
          ok: false,
...
Apply this clarification while keeping changes scoped to the existing objective boundaries.` : objectiveInstruction || answerText;
  ...
  validateObjectiveReservation(input) {
    const row = this.db.prepare(`SELECT session_id AS sessionId, run_id AS runId, snapshot_id AS snapshotId, status
         FROM autonomy_objectives
         WHERE id = ?`).get(input.objectiveId);
    if (!row)
      return { ok: false, reason: "autonomy objective reservation not found" };
    if (row.sessionId !== input.sessionId || row.runId !== input.runId || row.snapshotId !== input.snapshotId) {
  ...
  reconcileGatedObjectiveReservations(nowIso = asIsoNow(), staleAfterMs = 10 * 60000) {
    const rows = this.db.prepare(`SELECT id, created_at AS createdAt
         FROM autonomy_objectives
         WHERE status = 'gated'`).all();
    let linked = 0;
    let failed = 0;
    const nowMs = Date.parse(nowIso);
    const tx = this.db.transaction(() => {
      for (const row of rows) {
        ...
        if (Number.isFinite(nowMs) && Number.isFinite(createdMs) && nowMs - createdMs >= Math.max(60000, staleAfterMs)) {
          this.db.prepare(`UPDATE autonomy_objectives
               SET status = 'failed', block_reason = 'stale_objective_reservation', updated_at = ?
               WHERE id = ? AND status = 'gated'`).run(nowIso, row.id);
          failed += 1;
        }
      }
    });
...
`);
      ...
      const autonomyMetadataFromPayload = (value) => {
      ...
      const makeAutonomyReservationResponse = (value) => {
        ...
        const reservationRequired = autonomyMetadata.reservationRequired === true || autonomyMetadata.reservation_required === true;
        ...
        if (!reservationIdentity.objectiveId || !reservationIdentity.sessionId || !reservationIdentity.runId || !reservationIdentity.snapshotId || actualIdempotencyKey !== expectedIdempotencyKey) {
          return makeJson({
            ...
            message: "A matching autonomy objective reservation and idempotency key are required."
          }, 409);
        }
