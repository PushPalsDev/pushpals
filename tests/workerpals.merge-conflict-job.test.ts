...
async function createConflictFixture(options?: {
  branchCommitContents?: string[];
  mainContent?: string;
  conflictFile?: string;
  ...
  for (const [index, content] of branchCommitContents.entries()) {
    writeFileSync(join(maintainer, conflictFile), content, "utf8");
    await mustGit(maintainer, ["add", "-A"], `stage branch change ${index + 1}`);
    await mustGit(
      maintainer,
      ["commit", "-m", `branch change ${index + 1}`],
      `commit branch change ${index + 1}`,
    );
  }
  await mustGit(
    maintainer,
...
describe("workerpals merge-conflict sandbox", () => {
      ...
      try {
        ...
        try {
          ...
          const status = await mustGit(
            prepared.repoPath,
            ["status", "--porcelain"],
            "inspect chained merge-conflict repo after execution",
          );
          expect(status).toBe("");
          const resolvedFile = readFileSync(join(prepared.repoPath, fixture.conflictFile), "utf8");
          expect(resolvedFile).toContain("resolved-two");
          expect(resolvedFile).toContain("extra = 'branch'");
        } finally {
          prepared.cleanup();
        }
      } finally {
  ...
  runMergeConflictTest(
    "executeJob soft-passes repo validation blockers for publishable merge-conflict jobs without a revision retry",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "pushpals-merge-conflict-quality-soft-pass-"));
      const repo = join(root, "repo");
      mkdirSync(join(repo, "components", "__tests__"), { recursive: true });
      const testPath = "components/__tests__/AnimatedSelectionRing.test.tsx";
      const restoreSegments = stubBackendScriptSegmentsForTesting(TEST_BACKEND);
      ...
      try {
        ...
        await mustGit(repo, ["add", "-A"], "stage quality soft-pass repo");
        await mustGit(repo, ["commit", "-m", "seed test repo"], "commit quality soft-pass repo");
        ...
        expect(result.ok).toBe(true);
        expect(result.summary).toContain("quality gate soft-pass on repo blocker");
        expect(result.summary).not.toContain("Quality gate blocked by repo issue");
        expect(result.stderr).toContain("Cannot find module '../../tests/reactNativeMock'");
        expect(
          forwardedLogs.some((entry) =>
            entry.line.includes("Soft-pass on repo blocker for publishable merge_conflict job"),
          ),
        ).toBe(true);
        expect(
          forwardedLogs.some((entry) => entry.line.includes("Quality gate requested revision")),
