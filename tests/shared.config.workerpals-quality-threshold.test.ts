...
describe("shared config workerpals quality critic threshold parsing", () => {
  ...
  test("defaults openai_codex LLM services to GPT-6 Astra with extra-high reasoning", () => {
    const root = mkdtempSync(join(tmpdir(), "pushpals-config-"));
    const configDir = join(root, "configs");
    mkdirSync(configDir, { recursive: true });
    ...
    try {
      const cfg = loadPushPalsConfig({ projectRoot: root, reload: true });
      expect(cfg.localbuddy.llm.model).toBe("gpt-6-astra");
      expect(cfg.remotebuddy.llm.model).toBe("gpt-6-astra");
      expect(cfg.workerpals.llm.model).toBe("gpt-6-astra");
