...
import {
  AutonomyStore,
  deriveOutcomeFromPrFeedbackVerdict,
  normalizePrFeedbackVerdict,
} from "../apps/server/src/autonomy";
...
describe("server AutonomyStore policy gates", () => {
  ...
  describe("PR feedback verdict normalization", () => {
    test("maps positive verdicts to success outcomes", () => {
      expect(normalizePrFeedbackVerdict("Approved")).toBe("approved_merged");
      expect(normalizePrFeedbackVerdict("approved-unmergeable")).toBe("approved_unmergeable");
      const mergedOutcome = deriveOutcomeFromPrFeedbackVerdict("approved_merged");
      expect(mergedOutcome?.success).toBe(true);
      expect(mergedOutcome?.userAction).toBe("approved_merged");
      const unmergeableOutcome = deriveOutcomeFromPrFeedbackVerdict("approved_unmergeable");
      expect(unmergeableOutcome?.success).toBe(true);
      expect(unmergeableOutcome?.userAction).toBe("approved_unmergeable");
    });

    test("maps rejection variants to negative outcomes", () => {
      expect(normalizePrFeedbackVerdict("merge_failed")).toBe("rejected");
      expect(normalizePrFeedbackVerdict("rejected comment cap closed")).toBe(
        "rejected_comment_cap_closed",
      );
      const rejectedOutcome = deriveOutcomeFromPrFeedbackVerdict("merge_failed");
      expect(rejectedOutcome?.success).toBe(false);
      expect(rejectedOutcome?.userAction).toBe("rejected");
      const capOutcome = deriveOutcomeFromPrFeedbackVerdict("rejected_comment_cap_closed");
      expect(capOutcome?.success).toBe(false);
      expect(capOutcome?.userAction).toBe("rejected_comment_cap_closed");
    });

    test("treats unknown verdicts as neutral", () => {
      expect(normalizePrFeedbackVerdict("mystery verdict")).toBe("unknown");
      expect(deriveOutcomeFromPrFeedbackVerdict("mystery verdict")).toBeNull();
    });
  });
});
