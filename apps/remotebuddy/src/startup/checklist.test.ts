...
describe("StartupChecklist", () => {
  ...
  describe("telemetry safety", () => {
    test("safe logging keeps circular and bigint errors serializable", async () => {
      const preflightLogs: StartupCheckRecord[] = [];
      const ctx = buildContext({
        describeRepo: async () => {
          const circular: any = { reason: "circular" };
        ...
        log: (record) => {
          ...
          preflightLogs.push(record);
        },
      });
      const preflight = await runStartupPreflight(ctx);
      expect(preflight.ok).toBe(false);
      const failureRecord = preflightLogs.find((entry) => entry.status === "fail");
      expect(failureRecord?.error?.raw).toContain("[Circular]");
      expect(failureRecord?.error?.raw).toContain("[bigint 42]");
      expect(failureRecord?.error?.raw).toContain("circular");

      const dispatchLogs: StartupCheckRecord[] = [];
      const dispatchResult = await gateDispatchWithStartupPreflight(
        buildContext({
          log: (record) => {
            expect(() => JSON.stringify(record)).not.toThrow();
        ...
        async () => {
          const failure: any = { message: "dispatch circular" };
      ...
      const dispatchFailure = dispatchLogs.find(
        (entry) => entry.code === STARTUP_FAILURE_CODES.DISPATCH_FAILED,
      );
      expect(dispatchFailure?.error?.raw).toContain("[Circular]");
      expect(dispatchFailure?.error?.raw).toContain("[bigint 96]");
    });

    test("function normalization never leaks source text", async () => {
      const ctx = buildContext({
        describeRepo: async () => {
          const anonymous = () => {
            return "secret path leak";
          };
          throw anonymous;
        },
      });
      ...
      expect(record.error?.raw).toBe("[function anonymous]");
      expect(record.error?.message).toBe("[function anonymous]");
      expect(record.detail).toBe("[function anonymous]");
      expect(record.error?.raw?.includes("secret path leak")).toBe(false);

      const nestedCtx = buildContext({
        describeRepo: async () => {
          const named = function confidentialFunction() {
            return "super secret body";
          };
          const nested = {
            deeper: function deeperSecret() {
              return "nested secret path";
            },
          };
          throw {
            reason: named,
            nested,
            detail: "should respect placeholders",
          };
        },
      });
      const nestedResult = await runStartupPreflight(nestedCtx);
      const nestedRecord = nestedResult.history[0];
      const raw = nestedRecord.error?.raw ?? "";
      expect(raw).toContain("[function confidentialFunction]");
      expect(raw).toContain("[function deeperSecret]");
      expect(raw.includes("super secret body")).toBe(false);
      expect(raw.includes("nested secret path")).toBe(false);
    });

    test("stack telemetry is redacted by default and bounded when enabled", async () => {
      ...
      const redacted = await runStartupPreflight(ctx);
      const redactedStack = redacted.history[0].error?.stack;
      expect(redactedStack).toBe("[redacted]");
      const redactedRaw = redacted.history[0].error?.raw ?? "";
      expect(redactedRaw.includes("at fn (file.js:1:1)")).toBe(false);
      expect(redactedRaw).toContain("[redacted]");

      const debugResult = await runStartupPreflight(ctx, {
        includeErrorStack: true,
      });
      const record = debugResult.history[0];
      const stack = record.error?.stack ?? "";
      expect(stack).not.toBe("[redacted]");
      expect(stack.length).toBeLessThan(longStack.length);
      expect(stack.endsWith(" ...[truncated]")).toBeTruthy();
      const raw = record.error?.raw ?? "";
      expect(raw.length).toBeLessThan(2000);
      expect(raw.endsWith(" ...[truncated]")).toBeTruthy();
      expect(raw.includes("[redacted]")).toBe(false);
      expect(raw.includes("at fn (file.js:1:1)")).toBe(true);
    });
  });
});
