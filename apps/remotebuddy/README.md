...
4. **Recent automation (Jan–Feb 2026):**
   ...
   - Queue-health automation tags each synthetic request with explicit `write_globs` and `target_paths` as relevance hints. Operators should leave those untouched unless the hints point at the wrong area; override only when corrections are verified, then reply in the originating request so the engine stops re-firing.
