...
- **Dispatcher telemetry stream** – RemoteBuddy now emits both `[dispatcher.telemetry] …` log lines **and** `dispatcher_telemetry` session events (subscribe via `/sessions/:id/ws`) that capture queue-wait samples, dispatch-latency percentiles, concurrency, backpressure state, and current intake-throttle health. When the dispatcher’s rolling p95 **dispatch latency** exceeds **1.5 s** for ≥5 samples the stream reports `code=dispatcher.backpressure.latency_p95_high`, halts autonomous submissions, and throttles dispatcher intake so queue waits stay under 2 s until p95 recovers ≤1.0 s or the 60 s hold expires. The controller now tracks an explicit phase (`active_hold` vs `recovery_pending`) plus ingestion reason codes, and emits every 15 s while throttled so dashboards stay current even without new dispatch completions. Use the structured event payload to feed dashboards/alerts while the log line remains a quick CLI indicator. The event body exposes `queueWaitMs.*`, `dispatchLatencyMs.*`, `concurrency`, `backpressure.{code,phase,lastObservedLatencyP95Ms,releaseEligibleAtMs}`, and `ingestion.{throttleActive,holdRemainingMs,reasonCode}` so on-call engineers know exactly when intake/backoff is active.
