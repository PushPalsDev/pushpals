...
- **Telemetry watchlist** – Keep `/system/status` tailing so you can see `slo.requests.queueWaitMs.p95` (aka `queue_p95`) alongside `queues.requestPriorities` (per-lane backlog) and `workers.idle`, and pin Grafana’s WorkerPals Job Outcomes panel for `job_failure_rate` (task.execute failures / total jobs in the last 10 minutes). Pair those with `sig_queue_health` logs so autonomous spikes are surfaced even when dashboards lag.
...
| --- | --- | --- | --- |
| `queue_p95` | ≥ 1.0 s for 3 polls **or** pending interactive ≥ 15 | ≥ 1.5 s for 5 min **or** queue depth > 60 / idle workers < 6 total for 5 polls | Freeze background/eval submissions, confirm automation injected remediation jobs, and add WorkerPal capacity until `/system/status.workers.idle` ≥ 6 (≈ 2 per lane). |
...
- **Fast load-spike detection tips** – Run `watch -n 5 curl -sS $SERVER/system/status | jq '{queue_p95: .slo.requests.queueWaitMs.p95, pendingInteractive: .queues.requestPriorities.interactive, jobPendingSnapshot: .queues.jobPendingSnapshot, idleWorkers: .workers.idle}'` during incidents so you catch >0.5 s jumps before alerts aggregate, and keep `tail -f logs/remotebuddy.log | rg sig_queue_health` open to spot remediation bursts. When `queue_p95` jumps ≥0.3 s between polls, immediately check `/requests?status=pending&limit=20` for aging interactive prompts and manually re-prioritize them.
...
Use this table as the single source for queue wait, backlog, and idle worker thresholds (mirrored verbatim across RemoteBuddy queue docs).
...
- Run `curl -sS $SERVER/system/status | jq '{queue_p95: .slo.requests.queueWaitMs.p95, pendingInteractive: .queues.requestPriorities.interactive, jobPendingSnapshot: .queues.jobPendingSnapshot, idleWorkers: .workers.idle}'` → confirm `pendingInteractive`, `queue_p95`, and `jobPendingSnapshot` stay at/near zero; log anomalies in the ops doc.
...
- `curl -sS /workers` → maintain ≥ 6 idle workers total (≈ 2 per lane). If automation hasn’t reached `maxWorkers`, start another `bun run workerpals` instance locally or in the pool.
...
2. **WorkerPals/Platform secondary:** ping the platform on-call (same rotation as WorkerPals) if queue guardrails enter “Degradation” or “Incident” bands for >10 minutes, or if auto-spawn cannot push `/system/status.workers.idle` back to ≥ 6.
...
3. **Run non-runtime preflight checks** – Keep RemoteBuddy stopped while you validate supporting artifacts. Execute both commands below (in order) before touching any runtime:
   - `bun run docs:validate` — ensures every queue guardrail table, `/system/status` snippet, and schema reference matches the canonical source.
   - `PUSHPALS_AUTH_TOKEN=<token> bun run smoke` (or rely on `.env`) — proves the enqueue → claim → complete loop still works end-to-end.
   Fix any failure here so later rollbacks do not mask doc/spec drift.
4. **Start RemoteBuddy runtime** – Prefer the repo-root script `bun run remotebuddy` (executes `protocol:build` first, then `remotebuddy:only`). When protocols are already current, `bun run remotebuddy:only` or `bun --cwd apps/remotebuddy run start` (enables `remotebuddy_supervisor.ts` restarts) keeps downtime minimal. Keep the terminal or tmux pane open so you can tail logs live.
5. **Validate the new instance** – Wait for `[RemoteBuddy] Starting polling loop…` followed by at least one `claim payload` log. Immediately hit `curl -sS $SERVER/system/status | jq '{queue_p95: .slo.requests.queueWaitMs.p95, pendingInteractive: .queues.requestPriorities.interactive, jobPendingSnapshot: .queues.jobPendingSnapshot, idleWorkers: .workers.idle}'` and `curl -sS "$SERVER/requests?status=claimed&limit=5"`. Optionally enqueue a single interactive request to prove round-trip health. Confirm `queue_p95`, `pendingInteractive`, worker idle counts, and the CommunicationManager WebSocket reconnect cleanly.
6. **Observe, rollback if needed** – Watch logs for 5–10 minutes. If failures persist, stop the process, `git checkout <last-known-good>`, repeat the preflight checks from step 3, rerun step 4, and move `remotebuddy-state.db` aside only when it is clearly corrupted (RemoteBuddy will recreate it, but previously handled events may replay once). Document the outcome and handoff time in `#pushpals-ops`.
...
4. **Inspect overall runtime health**
   ...
   curl -sS "http://localhost:3001/system/status" \
    -H "Authorization: Bearer $PUSHPALS_AUTH_TOKEN" | jq '{queue_p95: .slo.requests.queueWaitMs.p95, pendingInteractive: .queues.requestPriorities.interactive, jobPendingSnapshot: .queues.jobPendingSnapshot, idleWorkers: .workers.idle}'
   # Expect `pendingInteractive: 0` once the smoke item clears; jobPendingSnapshot stays empty unless RemoteBuddy enqueued WorkerPal jobs
   ```
