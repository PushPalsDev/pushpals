...
const REVIEW_AGENT_COMMENT_MARKER = "## reviewagent:";

function isReviewAgentRemediationComment(comment: PullRequestComment): boolean {
  const normalizedBody = normalizeCommentBody(comment.body).toLowerCase();
  if (!normalizedBody.startsWith(REVIEW_AGENT_COMMENT_MARKER)) return false;
  return !normalizedBody.includes("changes approved");
}

function countReviewAgentRemediationComments(comments: PullRequestComment[]): number {
  let count = 0;
  for (const comment of comments) {
    if (isReviewAgentRemediationComment(comment)) count += 1;
  }
  return count;
}
...
export class ReviewAgent {
  ...
  private async rejectPr(pr: GitHubPR, verdict: ReviewVerdict, diff: string): Promise<boolean> {
    ...
    const remediationCommentCount = countReviewAgentRemediationComments(recentComments);
    if (remediationCommentCount >= maxPrCommentsBeforeGiveUp) {
      return await this.giveUpOnRejectedPr(pr, verdict, {
        jobId,
        sessionId,
        recentComments,
        maxPrCommentsBeforeGiveUp,
        remediationCommentCount,
      });
    }
  ...
  private async giveUpOnRejectedPr(
    ...
    context: {
      ...
      remediationCommentCount: number;
    },
  ): Promise<boolean> {
    this.deps.logWarn(
      `[${ts()}] [ReviewAgent] PR #${pr.number} reached ReviewAgent remediation comment cap (${context.remediationCommentCount}/${context.maxPrCommentsBeforeGiveUp}); closing without merge (scanned ${context.recentComments.length} recent comments).`,
    );
