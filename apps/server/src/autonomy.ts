...
function semanticObjectiveClusterKey(
  ...
  const visionObjectiveId = normalizeObjectiveClusterText(
    record.visionObjectiveId ??
      ...
      evidencePortfolio.vision_objective_id,
  );
...
const AUTONOMY_WORKER_HANDOFF_STALE_MS = 15 * 60_000;
...
export interface AutonomyOpsSummary {
  ...
  reliability: AutonomyReliabilityMetrics;
}

export type AutonomyAttemptOutcome =
  | "succeeded"
  | "product_quality_failed"
  | "validation_blocked"
  | "environment_blocked"
...
export interface AutonomyReliabilityMetrics {
  windowHours: number;
  attemptsTotal: number;
  outcomeCounts: Record<AutonomyAttemptOutcome, number>;
...
function normalizedAutonomyFailureTargets(params: Record<string, unknown>): string[] {
  const planning = asObject(params.planning);
  const autonomy = asObject(params.autonomy);
  return [
    asString(params.path),
    asString(params.targetPath ?? params.target_path),
    ...asStringArray(params.paths),
    asString(planning.targetPath ?? planning.target_path),
    ...asStringArray(planning.targetPaths ?? planning.target_paths),
    asString(autonomy.targetPath ?? autonomy.target_path),
    ...asStringArray(autonomy.targetPaths ?? autonomy.target_paths),
  ]
    .map((entry) =>
      entry
        .replace(/\\/g, "/")
...
function normalizedValidationEvidenceValues(values: string[], maxItems: number): string[] {
  return [...new Set(values.map(normalizeTrustedValidationFingerprintLine).filter(Boolean))]
    .sort((left, right) => left.localeCompare(right))
    .slice(0, maxItems);
}

function normalizeValidationEvidenceMetadata(
  metadata: Record<string, unknown>,
): Record<string, unknown> {
  const nested = asObject(
    metadata.validationEvidence ?? metadata.validation_evidence ?? metadata.evidence,
  );
  const values = (...candidates: unknown[]): string[] =>
    candidates.flatMap((candidate) => asStringArray(candidate));
  return {
    ...metadata,
    ...nested,
    ...
    targetPathHints: values(
      nested.targetPathHints,
      nested.target_path_hints,
      nested.affectedPaths,
      nested.affected_paths,
      metadata.targetPathHints,
      metadata.target_path_hints,
    ),
    failureLines: values(
      nested.failureLines,
      nested.failure_lines,
      nested.diagnostics,
      metadata.failureLines,
      metadata.failure_lines,
    ),
    failureFingerprint:
      asString(
        nested.failureFingerprint ??
          nested.failure_fingerprint ??
          nested.fingerprint ??
          metadata.failureFingerprint ??
          metadata.failure_fingerprint,
      ) || undefined,
    retryReason:
      asString(
        nested.retryReason ?? nested.retry_reason ?? metadata.retryReason ?? metadata.retry_reason,
      ) || undefined,
  };
}
...
function canonicalValidationFailureEvidence(params: {
  ...
  const output = `${asString(params.stderrTail)}\n${asString(params.stdoutTail)}`;
  const derived = extractTrustedValidationFailureEvidence({
    command: params.command,
    phase: asString(metadata.phase) === "dependency_install" ? "dependency_install" : "validation",
    output,
    exitCode: 1,
  });
  const failedTests = normalizedValidationEvidenceValues(
    [...asStringArray(metadata.failedTests), ...derived.failedTests],
    20,
  );
  const targetPathHints = normalizedValidationEvidenceValues(
  ...
  if (failureLines.length === 0) {
    failureLines = normalizedValidationEvidenceValues(
      output
        .split(/\r?\n/)
        .filter((line) => /\b(?:error|fail|failed|failure|fatal|panic|timeout)\b/i.test(line)),
      4,
    );
  }
  ...
  const fingerprint = sha256Hex(
    JSON.stringify({
      ...
      failureClass: asString(params.failureClass).toLowerCase() || "unknown_failure",
      failedTests,
      targetPathHints,
      // Free-form diagnostics often contain prompt wording, timestamps, and
      // worktree identity. They are only a fallback when the validation
      // boundary could not extract a stable test or target path.
...
function validationRunTargetsBaseline(metadata: Record<string, unknown>): boolean {
  const target = asString(metadata.validationTarget ?? metadata.validation_target).toLowerCase();
  return (
    target === "baseline" ||
    asBoolean(metadata.baselineFailureProven ?? metadata.baseline_failure_proven, false)
  );
}
...
function isNonRepairableValidationEnvironmentFailure(
  ...
  if (
    normalizedClass === "environment" ||
    normalizedClass === "trusted_validation_required" ||
    normalizedClass === "dependency_setup_failed" ||
    normalizedClass === "timeout" ||
  ...
  return (
    ...
    /\b(?:econnreset|econnrefused|etimedout|enotfound|network is unreachable|could not resolve host|temporary failure|temporarily unavailable|tls handshake|certificate verify|unable to verify|docker daemon|cannot connect to (?:the )?docker|resource busy|missing runtime|credential|permission denied)\b/i.test(
      text,
    )
  );
}
...
function classifyAutonomyAttemptOutcome(input: {
  ...
  summary: string | null;
}): AutonomyAttemptOutcome {
  const status = asString(input.status).toLowerCase();
  const failureClass = asString(input.failureClass).toLowerCase();
  ...
  if (
    /critic[_ -]?rejected|quality[_ -]?(?:rejected|failed|revision[_ -]?exhausted)|revision[_ -]?budget[_ -]?exhausted|deterministic[_ -]?quality[_ -]?failed/.test(
      text,
    )
  ) {
  ...
  if (
    /trusted[_ -]?validation|validation[_ -]?(?:blocked|failed)|test[_ -]?failure|lint[_ -]?failure|typecheck[_ -]?failure/.test(
      text,
    )
  ) {
  ...
  if (
    ...
    /publication|publish[_ -]?(?:blocked|failed)|push[_ -]?failed|merge[_ -]?failed/.test(text)
  ) {
...
export class AutonomyStore {
  ...
  private getResourceUsageLastHour(nowIso: string): {
    ...
    const tokenRow = this.db
      ...
      .get(nowIso) as { total_tokens: number | null } | undefined;
    ...
    if (this.hasTable("jobs")) {
      const activeRuntimeRow = this.db
        .prepare(
          `SELECT SUM(
             MIN(
               3600000,
        ...
        .get(nowIso) as { active_runtime_ms: number | null } | undefined;
  ...
  private safetyBlockReason(
    nowIso = asIsoNow(),
    options: { allowFrozenRecovery?: boolean } = {},
  ): string | null {
  ...
  private recordOpsAlert(params: {
    ...
    this.db
      .prepare(
        `INSERT INTO autonomy_ops_alert_state (
      ...
      .run(alertType, params.severity, message, detailsJson, nowIso, nowIso);
    this.db
      .prepare(
  ...
  private resolveOpsAlert(alertTypeRaw: string, nowIso = asIsoNow()): void {
    const alertType = asString(alertTypeRaw);
    if (!alertType) return;
    this.db
      .prepare(
        `UPDATE autonomy_ops_alert_state
      ...
      .run(nowIso, nowIso, alertType);
  }

  private listRecentOpsAlerts(limit = 20): AutonomyOpsAlert[] {
    const rows = this.db
      .prepare(
        `SELECT alert_type, status, severity, message, details_json,
                first_seen_at, last_seen_at, resolved_at, occurrence_count
         FROM autonomy_ops_alert_state
         WHERE status = 'open'
    ...
    return rows.map((row) => {
      ...
      return {
        id: `alert_state_${sha256Hex(asString(row.alert_type)).slice(0, 12)}`,
        alertType: asString(row.alert_type) || "generic",
        severity,
        message: asString(row.message),
        details: parseJsonObject(row.details_json),
        createdAt: asString(row.first_seen_at),
  ...
  private getAutonomyJobHealth(
    ...
    if (this.hasTable("job_validation_runs") && this.hasTable("job_terminal_diagnostics")) {
      ...
      for (const row of repeatedRows) {
        ...
        const autonomy = asObject(jobParams.autonomy);
        ...
        const persistedCluster = asString(autonomy.patternKey ?? autonomy.pattern_key);
        const objectiveCluster =
          semanticCluster || (persistedCluster.startsWith("cluster_") ? persistedCluster : "");
        const targets = normalizedAutonomyFailureTargets(jobParams);
        if (!objectiveCluster && targets.length === 0) continue;
        const fingerprint = sha256Hex(
          JSON.stringify({
            objectiveCluster: objectiveCluster || null,
            targets: objectiveCluster ? [] : targets,
            failureClass: asString(row.failure_class).toLowerCase() || "unknown",
            command: asString(row.failed_command).replace(/\s+/g, " ").toLowerCase(),
    ...
    return {
      terminalCount,
      successRate: terminalCount > 0 ? clamp01(successCount / terminalCount) : null,
  ...
  private getAutonomyReviewDeliveryHealth(
    ...
    const reviews = this.db
      .prepare(
        `SELECT COUNT(DISTINCT feedback.objective_id) AS reviewed_objective_count,
         ...
         WHERE objective.source = 'autonomous'
           AND datetime(feedback.created_at) >= datetime(?, '-${Math.max(1, windowHours)} hours')`,
      )
      .get(nowIso) as
      | { reviewed_objective_count: number | null; revision_objective_count: number | null }
      | undefined;
    ...
    const reviewedObjectiveCount = Math.max(
      0,
      Math.floor(asNumber(reviews?.reviewed_objective_count, 0)),
    );
    const revisionObjectiveCount = Math.max(
      0,
      Math.floor(asNumber(reviews?.revision_objective_count, 0)),
    );
    return {
      awaitingReviewCount,
      ...
      revisionRate:
        reviewedObjectiveCount > 0
          ? clamp01(revisionObjectiveCount / reviewedObjectiveCount)
          : null,
    };
  }
  ...
  private latestEvaluatorScorecard(): AutonomyEvaluatorScorecard | null {
    ...
    return {
      ...
      jobSuccessRate: Number.isFinite(asNumber(payload.jobSuccessRate, Number.NaN))
        ? clamp01(asNumber(payload.jobSuccessRate, 0))
        : null,
      jobTimeoutRate: Number.isFinite(asNumber(payload.jobTimeoutRate, Number.NaN))
        ? clamp01(asNumber(payload.jobTimeoutRate, 0))
        : null,
  ...
  private runEvaluator(nowIso = asIsoNow()): AutonomyEvaluatorScorecard {
    ...
    const infrastructureFailureRate =
      reliability.attemptsTotal > 0
        ? (reliability.outcomeCounts.orchestration_failed +
            reliability.outcomeCounts.environment_blocked) /
          reliability.attemptsTotal
        : null;
    ...
    if (resourceBudget.token_budget_exhausted || resourceBudget.runtime_budget_exhausted) {
      ...
      (hasOutcomeEvidence || hasReviewEvidence || reviewHealth.awaitingReviewCount >= minSamples)
    ) {
      recommendation = "pause";
      ...
      (hasOutcomeEvidence &&
        ((typeof successRate === "number" &&
          successRate < Math.min(1, Math.max(0.8, minSuccessRate + 0.08))) ||
    ...
    if (
      ...
      this.recordOpsAlert({
        ...
        message:
          ...
          `regretRate=${regretRate?.toFixed(2) ?? "n/a"} jobSuccessRate=${jobHealth.successRate?.toFixed(2) ?? "n/a"} ` +
          `jobTimeoutRate=${jobHealth.timeoutRate?.toFixed(2) ?? "n/a"} samples=${sampleCount}/${jobHealth.terminalCount}`,
        details: payload,
        nowIso,
      });
      if (!resourcePause) {
  ...
  getReliabilityMetrics(windowHours = 24, nowIso = asIsoNow()): AutonomyReliabilityMetrics {
    ...
    if (this.hasTable("jobs") && this.hasTable("job_terminal_diagnostics")) {
      attemptRows = this.db
        ...
        .all(nowIso) as typeof attemptRows;
    }
    const outcomeCounts: Record<AutonomyAttemptOutcome, number> = {
      succeeded: 0,
    ...
    const objectiveTerminalCount = Math.max(
      0,
      Math.floor(asNumber(objectiveRow?.terminalCount, 0)),
    );
    const objectiveSuccessCount = Math.max(0, Math.floor(asNumber(objectiveRow?.successCount, 0)));
    const nonTerminalRevisionCount = Math.max(
      0,
      Math.floor(asNumber(objectiveRow?.nonTerminalRevisionCount, 0)),
    );
    const nonTerminalRevisionObjectiveCount = Math.max(
      0,
      Math.floor(asNumber(objectiveRow?.nonTerminalRevisionObjectiveCount, 0)),
    );
    const revisedTerminalObjectiveCount = Math.max(
      0,
      Math.floor(asNumber(objectiveRow?.revisedTerminalObjectiveCount, 0)),
    );
    ...
    if (this.hasTable("job_validation_runs") && this.hasTable("jobs")) {
      validationRows = this.db
        .prepare(
          `SELECT v.jobId, v.command, v.attempt, v.passed, v.failureClass,
    ...
    for (const row of validationRows) {
      ...
      if (
        attempt >= 2 &&
        asString(metadata.retryReason ?? metadata.retry_reason) === "transient_infrastructure"
      ) {
        transientRetryKeys.add(`${row.jobId}\n${asString(row.command)}\n${attempt}`);
      }
      if (Number(row.passed) === 1) continue;
      if (failedTests.length > 0 || targetPathHints.length > 0 || failureLines.length > 0) {
        validationEvidenceCovered += 1;
      }
      const fingerprint = asString(metadata.failureFingerprint) || canonical.fingerprint;
      if (!fingerprint) continue;
      const structuredEvidenceAvailable = failedTests.length > 0 || targetPathHints.length > 0;
      const signature = JSON.stringify({
        failureClass: asString(row.failureClass),
        failedTests,
        targetPathHints,
        failureLines: structuredEvidenceAvailable ? [] : failureLines,
      });
      const signatures = signaturesByFingerprint.get(fingerprint) ?? new Set<string>();
      signatures.add(signature);
      signaturesByFingerprint.set(fingerprint, signatures);
    }
    const validationFingerprintCollisionCount = [...signaturesByFingerprint.values()].filter(
      (signatures) => signatures.size > 1,
    ).length;
    const handoffCutoff = new Date(
      Date.parse(nowIso) - AUTONOMY_WORKER_HANDOFF_STALE_MS,
    ).toISOString();
    const incidentRow = this.db
      ...
      .get(nowIso, handoffCutoff, nowIso) as {
      handoffFailures: number | null;
      stalledHandoffs: number | null;
    } | null;
    ...
    return {
      ...
      objectiveSuccessRate:
        objectiveTerminalCount > 0 ? objectiveSuccessCount / objectiveTerminalCount : null,
      ...
      durationMs: {
        average:
          durations.length > 0
            ? Math.floor(durations.reduce((sum, value) => sum + value, 0) / durations.length)
            : null,
  ...
  getOpsSummary(params?: {
    ...
    if (
      params?.jobFailureRate !== undefined &&
      jobFailureRate >= failureThreshold &&
      failureThreshold > 0
    ) {
      this.recordOpsAlert({
  ...
  private detectActiveValidationIncident(nowIso: string): ValidationIncident | null {
    ...
    for (const row of rows) {
      ...
      const baselineSha = asString(metadata.baselineSha) || null;
      const candidateSha = asString(metadata.candidateSha) || null;
      const candidateRef = asString(metadata.candidateRef) || null;
      ...
      const failureFingerprint = evidence.fingerprint;
      ...
      const group =
        ...
        ({
          ...
          failureLines: new Set<string>(),
          baselineSha,
          baselineFailureProven: false,
          candidateShas: new Set<string>(),
          candidateRefs: new Map<string, string>(),
      ...
      if (baselineTarget) group.baselineFailureProven = true;
      if (candidateSha && !baselineTarget) group.candidateShas.add(candidateSha);
      if (candidateSha && candidateRef) group.candidateRefs.set(candidateSha, candidateRef);
    ...
    const activeGroups = [...groups.values()]
      ...
      .filter((group) => {
        ...
        const passKey =
          group.source === "worker"
            ? `${commandSourceKey}\nworker`
            : group.baselineFailureProven
              ? `${commandSourceKey}\nbaseline`
              : group.latestFailureCandidateSha
                ? `${commandSourceKey}\ncandidate:${group.latestFailureCandidateSha}`
                : `${commandSourceKey}\nunknown-candidate`;
    ...
    const exactCandidateSha =
      (latestCandidateSha && selected.candidateRefs.has(latestCandidateSha)
        ? latestCandidateSha
        : candidateShas.find((sha) => selected.candidateRefs.has(sha))) ?? null;
    const validationScope =
      selected.source === "worker"
        ? "worker_local"
        : selected.baselineFailureProven
          ? "baseline_suspected"
          : exactCandidateSha
            ? "candidate_specific"
            : "candidate_unavailable";
    const evidenceQuality =
      selected.failedTests.size > 0 && selected.pathHints.length > 0
        ? "high"
        : selected.failureLines.size > 0 || selected.pathHints.length > 0
          ? "medium"
          : "low";
    return {
      active: true,
      ...
      candidate_ref: exactCandidateSha
        ? (selected.candidateRefs.get(exactCandidateSha) ?? null)
        : null,
  ...
  private buildTopSignals(
    ...
    const jobFailures =
      Number(jobSlo?.failed ?? 0) +
      Number(jobSlo?.abandoned ?? 0) +
      Number(jobSlo?.publishBlocked ?? 0);
  ...
  createSnapshot(params: {
    ...
    type ObjectiveSnapshotRow = Omit<OpenObjective, "target_paths" | "scope"> & {
      ...
      job_validation_stdout_tail: string | null;
      job_validation_stderr_tail: string | null;
      job_validation_metadata_json: string | null;
    };
    const includeObjectiveJobOutcomes =
      this.hasTable("jobs") && this.hasTable("job_terminal_diagnostics");
    ...
    const objectiveValidationProjection = includeObjectiveValidationOutcomes
      ? `v.passed AS job_validation_passed,
         ...
         v.metadataJson AS job_validation_metadata_json`
      : `NULL AS job_validation_passed,
    ...
    const objectiveValidationJoins = includeObjectiveValidationOutcomes
      ? `LEFT JOIN job_validation_runs v ON v.id = (
    ...
    const hydrateObjective = (row: ObjectiveSnapshotRow): OpenObjective => {
      ...
      const attemptOutcome = terminalJobStatuses.has(asString(row.job_status))
        ? classifyAutonomyAttemptOutcome({
            status: asString(row.job_status),
            failureClass: row.job_failure_class,
            terminalStage: row.job_terminal_stage,
            summary: row.job_summary,
          })
        : null;
      const deterministicRepairFailure =
        attemptOutcome === "product_quality_failed" ||
        attemptOutcome === "validation_blocked" ||
        attemptOutcome === "no_change";
      const validationCommand = asString(row.job_validation_command);
      const attemptFailureFingerprint =
        deterministicRepairFailure && validationCommand && Number(row.job_validation_passed) !== 1
          ? canonicalValidationFailureEvidence({
  ...
  evaluateEligibility(body: Record<string, unknown>): {
    ...
    const results = candidates.map((raw) => {
      ...
      const patternKey = semanticObjectiveClusterKey(
        record,
        asString(record.patternKey ?? record.pattern_key),
      );
  ...
  recordObjectiveDecision(body: Record<string, unknown>): {
    ...
    const objectiveCandidateId = objectiveCandidateRaw
      ...
      : null;
    const portfolioMeta =
      (objectiveCandidateId ? candidatePortfolioMetaById.get(objectiveCandidateId) : undefined) ??
      {};
    const patternKey = semanticObjectiveClusterKey(
      objective,
      makePatternKey(
    ...
    const incidentKey = asString(
      objective.incidentKey ??
        objective.incident_key ??
        validationIncidentEvidence.incident_id ??
        validationIncidentEvidence.incidentId ??
        validationIncidentEvidence.failure_fingerprint ??
        validationIncidentEvidence.failureFingerprint,
    ).slice(0, 256);
    if (
      incidentKey &&
      [
      ...
      const existing = this.db
        ...
        .get(incidentKey, objectiveId) as { id: string } | null;
      if (existing?.id) {
        return {
          ok: false,
        ...
        candidates: [
          {
            ...
            lifecycle_recovery:
              objective.lifecycleRecovery ?? objective.lifecycle_recovery ?? false,
          },
        ],
      });
      if (!eligibility.ok) {
    ...
    if (repositoryAgentRequestId && this.hasTable("repository_agent_requests")) {
      ...
      if (
        ...
        try {
          ...
          const memoryRefs = asRepositoryAgentMemoryRefs(result.memoryRefs).filter(
            (ref) => ref.key,
          );
          if (
            repositoryId &&
  ...
  recordPrFeedback(body: Record<string, unknown>): {
    ...
    if (mappedOutcome?.terminal && hasJobsTable && (!jobIdRaw || !normalizedFeedbackPrUrl)) {
      return acknowledgePermanentIgnore(
        "terminal PR feedback requires matching jobId and prUrl authority",
      );
    }
    ...
    if (jobIdRaw && normalizedFeedbackPrUrl && hasJobsTable) {
      ...
      if (persistedJobPrUrl && persistedJobPrUrl !== normalizedFeedbackPrUrl) {
        return acknowledgePermanentIgnore(
          "PR feedback URL does not match the persisted job PR URL",
        );
      }
      ...
      if (mappedOutcome?.terminal && !mappedOutcome.success) {
        ...
        if (!latestJob || latestJob.id !== jobIdRaw) {
          return acknowledgePermanentIgnore(
            "closed PR feedback belongs to an older job generation",
          );
        }
      }
    }
    ...
    if (identityMismatch) {
      return acknowledgePermanentIgnore(
        "PR feedback identifiers do not match the persisted job context",
      );
    }
    if (!patternKey) {
    ...
    if (!patternKey) {
      ...
      return acknowledgePermanentIgnore(
        "unable to resolve patternKey from objectiveId/requestId/jobId/prUrl",
      );
    }
  ...
  private recordOutcomeMutation(body: Record<string, unknown>): { ok: boolean; reason?: string } {
    ...
    const reportedLatencyMs = Number.isFinite(
      asNumber(body.latencyMs ?? body.latency_ms, Number.NaN),
    )
      ? Math.max(0, Math.floor(asNumber(body.latencyMs ?? body.latency_ms, 0)))
      : null;
    const lifecycleStart = objectiveId
      ? (this.db
          .prepare(
            `SELECT created_at AS createdAt, dispatched_at AS dispatchedAt
             FROM autonomy_objectives
             WHERE id = ?
             LIMIT 1`,
          )
          .get(objectiveId) as
          | { createdAt: string | null; dispatchedAt: string | null }
          | undefined)
      : undefined;
    const jobStart =
      jobId && this.hasTable("jobs")
        ? (this.db
            .prepare(`SELECT enqueuedAt, createdAt FROM jobs WHERE id = ? LIMIT 1`)
            .get(jobId) as { enqueuedAt: string | null; createdAt: string | null } | undefined)
        : undefined;
    ...
    const endToEndLatencyMs =
      startCandidates.length > 0 && Number.isFinite(nowMs)
        ? Math.max(0, Math.floor(nowMs - Math.min(...startCandidates)))
        : null;
    const latencyMs =
      reportedLatencyMs != null && endToEndLatencyMs != null
        ? Math.max(reportedLatencyMs, endToEndLatencyMs)
        : (reportedLatencyMs ?? endToEndLatencyMs);
    ...
    if (!success && nextFailStreak >= autoFreezeThreshold) {
      // Repeated attempts in one objective/failure lane are not independent
      // evidence for a repository-wide stop. The pattern cooldown is the lane
      // circuit; only the evaluator may freeze globally after enough distinct
      // root objectives have terminal evidence.
      this.recordOpsAlert({
        alertType: `autonomy_lane_constrained:${patternKey}`,
        severity: "warning",
        message:
          `Autonomy lane constrained after fail streak ${nextFailStreak} ` +
  ...
  listInsights(params?: {
    ...
    const portfolio: AutonomyPortfolioMetrics = {
      ...
      firstPassMergeRate:
        reviewObservedObjectiveCount > 0
          ? firstPassMergeCount / reviewObservedObjectiveCount
          : null,
    };
  ...
  findObjectiveByJobId(jobId: string): {
    ...
    const row = this.db
      .prepare(
        `SELECT id AS objectiveId, pattern_key AS patternKey, request_id AS requestId,
  ...
  resolveJobOutcomeContext(
    ...
    const resolve = (
    ...
    ): { objectiveId: string | null; requestId: string | null; patternKey: string } | null => {
      if (!currentJobId || visited.has(currentJobId) || visited.size >= 16) return null;
      ...
      if (matched) {
        return {
          ...
          requestId:
            asString(currentParams?.requestId ?? currentParams?.request_id) ||
            asString(matched.requestId) ||
            null,
          patternKey: matched.patternKey,
        };
      }

      const row = this.hasTable("jobs")
        ? (this.db
            .prepare(`SELECT params, resumeOfJobId FROM jobs WHERE id = ? LIMIT 1`)
            .get(currentJobId) as
            | { params: string | null; resumeOfJobId: string | null }
            | undefined)
        : undefined;
      const resolvedParams =
        currentParams && Object.keys(currentParams).length > 0
          ? currentParams
          : parseJsonObject(row?.params ?? null);
      const autonomy = asObject(resolvedParams.autonomy);
      const origin = asString(resolvedParams.origin ?? autonomy.origin).toLowerCase();
      const patternKey = asString(autonomy.patternKey ?? autonomy.pattern_key);
      const localObjectiveId = asString(autonomy.objectiveId ?? autonomy.objective_id);
      const localRequestId = asString(resolvedParams.requestId ?? resolvedParams.request_id);
      ...
      const reviewAgent = asObject(resolvedParams.reviewAgent);
      const recovery = asObject(resolvedParams.recovery);
      const parents = [
        asString(reviewAgent.sourceJobId ?? reviewAgent.source_job_id),
        asString(reviewAgent.recoverySourceJobId ?? reviewAgent.recovery_source_job_id),
        asString(recovery.previousJobId ?? recovery.previous_job_id),
        asString(row?.resumeOfJobId),
      ].filter(Boolean);
      for (const parentJobId of parents) {
        const parent = resolve(parentJobId);
  ...
  markObjectiveHandoffFailed(requestId: string, reason: string): void {
    if (!requestId) return;
    const now = asIsoNow();
    this.db
      .prepare(
        `UPDATE autonomy_objectives
         ...
         WHERE request_id = ?
           ...
           AND status IN ('gated','dispatched')`,
      )
      .run(asString(reason).slice(0, 500) || "worker_handoff_failed", now, requestId);
  }

  reconcileObjectiveWorkerHandoffs(
    nowIso = asIsoNow(),
    staleAfterMs = AUTONOMY_WORKER_HANDOFF_STALE_MS,
  ): { linked: number; failed: number; pending: number; scanned: number } {
    if (!this.hasTable("requests") || !this.hasTable("jobs")) {
      return { linked: 0, failed: 0, pending: 0, scanned: 0 };
    }
    const rows = this.db
      .prepare(
        `SELECT o.id, o.session_id AS objectiveSessionId,
    ...
    let pending = 0;
    const nowMs = Date.parse(nowIso);
    const minimumAgeMs = Math.max(5_000, Math.floor(staleAfterMs));
    const tx = this.db.transaction(() => {
      for (const row of rows) {
        if (
          row.requestStatus &&
          asString(row.requestSessionId) !== asString(row.objectiveSessionId)
        ) {
          this.db
            .prepare(
              `UPDATE autonomy_objectives
               SET status = 'failed', block_reason = ?, updated_at = ?
               WHERE id = ? AND status = 'dispatched' AND (job_id IS NULL OR job_id = '')`,
            )
            .run("worker_handoff_request_session_mismatch", nowIso, row.id);
          failed += 1;
          continue;
        }
        const job = this.db
          ...
          .get(
            ...
            row.requestId,
            row.requestEnqueuedAt,
            asString(row.handoffJobId),
          ) as {
          id: string;
        } | null;
        if (job?.id) {
        ...
        const leaseExpiresAtMs = Date.parse(asString(row.leaseExpiresAt));
        const heartbeatAtMs = Date.parse(asString(row.lastHeartbeatAt));
        const leaseUnexpired =
          Number.isFinite(nowMs) && Number.isFinite(leaseExpiresAtMs) && leaseExpiresAtMs > nowMs;
        const heartbeatFresh =
          Number.isFinite(nowMs) &&
          Number.isFinite(heartbeatAtMs) &&
          nowMs - heartbeatAtMs < minimumAgeMs;
        const claimedLeaseLive =
          row.requestStatus === "claimed" &&
          (leaseUnexpired || (!Number.isFinite(leaseExpiresAtMs) && heartbeatFresh));
        const requestResult = parseJsonObject(row.requestResult);
        const requiresWorker =
          Number(row.workerRequired) === 1 || asBoolean(requestResult.requiresWorker, false);
        const requestTerminalWithoutHandoff =
          row.requestStatus === "failed" ||
          (row.requestStatus === "completed" && requiresWorker) ||
          !row.requestStatus;
        ...
        if (row.requestStatus === "claimed" && !claimedLeaseLive) {
          this.db
            ...
            .run(nowIso, row.requestId, nowIso);
          pending += 1;
          continue;
        }
        ...
        const reason =
          row.requestStatus === "completed" && requiresWorker
            ? "worker_handoff_missing_after_request_completion"
            : row.requestStatus === "failed"
              ? "worker_handoff_request_failed"
              : "worker_handoff_request_missing";
        this.db
          .prepare(
            `UPDATE autonomy_objectives
             SET status = 'failed', block_reason = ?, updated_at = ?
  ...
  pruneRepositoryAgentMemoryFeedback(
    ...
    const maxTerminalRows = Math.max(
      ...
      Math.min(
        1_000_000,
        Math.floor(
          asNumber(options.maxTerminalRows, REPOSITORY_AGENT_MEMORY_FEEDBACK_MAX_TERMINAL_ROWS),
        ),
      ),
    );
    const batchSize = Math.max(
      ...
      Math.min(
        5_000,
        Math.floor(asNumber(options.batchSize, REPOSITORY_AGENT_MEMORY_FEEDBACK_PRUNE_BATCH)),
      ),
    );
  ...
  reconcileRepositoryAgentMemoryFeedback(): { queued: number } {
    ...
    const eligibleNegativeJobRows = negativeJobRows.filter((row) => {
      ...
      return (
        directNegative ||
        ["product_quality_failed", "validation_blocked", "no_change"].includes(classified)
      );
    });
