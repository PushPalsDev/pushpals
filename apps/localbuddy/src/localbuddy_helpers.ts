...
export function isLikelyLocalOnlyPrompt(input: string): boolean {
  ...
  const executionCue =
    /\b(fix|implement|write|create|add|remove|delete|rename|refactor|write|run|test|lint|build|debug|search|find|edit|update|change)\b/.test(
      text,
    );
  if (executionCue) return false;

  // Short affirmatives should go to RemoteBuddy.
  if (
