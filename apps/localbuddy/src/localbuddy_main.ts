...
import { answerLocalReadonlyQuery } from "./local_readonly.js";
import {
...
export interface LocalBuddyHandlerOptions {
  agentId: string;
  repo: string;
  serverUrl: string;
  sessionId: string;
  authToken: string | null;
  comm: Pick<CommunicationManager, "assistantMessage" | "userMessage">;
  answerLocally: (prompt: string) => Promise<string>;
  fetchImpl?: typeof fetch;
}

export function createLocalBuddyFetchHandler(options: LocalBuddyHandlerOptions) {
  const {
    agentId,
    repo,
    serverUrl,
  ...
  return async function localBuddyFetchHandler(req: Request): Promise<Response> {
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              const res = await fetchImpl(`${serverUrl}/requests/enqueue`, {
                method: "POST",
                headers: cmdHeaders,
                body: JSON.stringify({
                  sessionId,
...
{"reply":"<final user-facing response>"}`,
  ...
  async startServer(port: number): Promise<void> {
    ...
    const fetchHandler = createLocalBuddyFetchHandler({
      agentId,
      repo,
      serverUrl,
      sessionId,
